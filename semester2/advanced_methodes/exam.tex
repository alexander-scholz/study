...
\fcolorbox{red}{white}{\parbox{\linewidth}{\textbf{Lemma:} Invarianten des Pebble Games mit Graph G, n Anzahl Knoten, v Knoten, Teilgraph V'$\subset$V, n' Anzahl Knoten in V'\\
(Invarianten: Eigenschaften die über das Spiel gleich bleiben)
\begin{enumerate}
	\item peb(v) + span(v) + out(v) = k
	\item peb(V') + span(V') + out(V') = $k \cdot |$n'$|$
	\item peb(V') + out(V') $\geq$ l (am Ende des Spiels bleiben mind. l Pebbles im Spielfeld)
	\item span(V')$\leq$k$|$n'$|$-l
\end{enumerate}
}}
\\\\
